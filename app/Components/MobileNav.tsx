...
const MobileNav = () => {
  ...
  return (
    ...
    <button onClick={displayNav} className='absolute right-10 block w-8 h-8 lg:hidden'>
      <span className={'absolute block w-full h-1 bg-black origin-left duration-500 top-0 '+`${mobile ? 'rotate-45 translate-y-1/2' : ""}`}></span>
      <span className={'absolute block w-full h-1 bg-black duration-500 top-1/2 -translate-y-1/2 '+`${mobile ? 'opacity-0': 'opacity-100'}`}></span>
      <span className={'absolute block w-full h-1 bg-black origin-left duration-500 bottom-0 '+`${mobile ? '-rotate-45 -translate-y-1/2' : ''}`}></span>
    </button>
